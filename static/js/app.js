// js/app.js — NeonPanel v3 SPA (single VLESS protocol). Author: OpenCode
...
const I={
  login:'ورود مدیر',pass:'رمز عبور',go:'وارد شو',bad:'رمز اشتباه است',
  dash:'داشبورد',cfgs:'کانفیگ‌ها',groups:'گروه‌ها',set:'تنظیمات',out:'خروج',
  online:'آنلاین',active:'فعال',total:'کل کانفیگ‌ها',used:'مصرف کل',
  newc:'＋ کانفیگ جدید',search:'جست‌وجو…',empty:'هنوز کانفیگی نساختی',
  name:'نام',status:'وضعیت',quota:'سقف (GB)',days:'روز',spd:'سرعت (Mbps)',ips:'IP هم‌زمان',
  usedb:'مصرف',expire:'انقضا',acts:'عملیات',
  on:'فعال',off:'قطع',noexp:'نامحدود',
  copy:'کپی',copied:'کپی شد ✅',qr:'QR',sub:'ساب',reset:'ریست مصرف',del:'حذف',
  askdel:'مطمئنی حذف شود؟',links:'لینک‌ها',view:'مشاهده صفحهٔ ساب',
  nname:'نام کانفیگ',nq:'سقف حجم GB (۰=نامحدود)',nd:'روزهای اعتبار (۰=بدون انقضا)',
  ...
  chp:'تغییر رمز',old:'رمز فعلی',new:'رمز جدید',save:'ذخیره',vers:'نسخه',
  err:'مشکلی پیش آمد',
};
...
const IC={
  ...
  cfgs:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M13 2L4 14h6l-1 8 9-12h-6z"/></svg>',
  groups:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="8" r="4"/><path d="M2 21c0-4 3.5-6 7-6s7 2 7 6"/><circle cx="17" cy="9" r="3"/><path d="M22 21c0-3-2-5-4.5-5"/></svg>',
...
async function pLogin(){
  ...
  <div class="brand">⚡ NeonPanel</div><h2 style="text-align:center">${I.login}</h2>
  <form id="f"><label>${I.pass}</label><input id="p" type="password" autofocus required>
  <button class="btn" style="width:100%;margin-top:14px">${I.go}</button></form></div></div>`;
  $('#f').onsubmit=async e=>{e.preventDefault();
    try{await api('/api/login',{method:'POST',body:JSON.stringify({password:$('#p').value})});location.hash='#/'}
    catch(ex){toast(ex.code==='LOCKED'?'قفل شدی — صبر کن':I.bad,'bad')}};
}

async function pDash(){
  const [st,live]=await Promise.all([api('/api/stats'),api('/api/live')]);
  ...
  <div class="card"><h2>${I.online} — ${live.length}</h2>
    ${live.length?live.map(l=>`<span class="badge on">⚡ ${l.uuid.slice(0,8)} · ${l.ips} IP</span> `).join(''):`<p style="color:var(--txt-1);font-size:13px">اتصال فعالی نیست</p>`}
  </div>
...
function cfgModal(){
  ...
  <label>${I.nname}</label><input id="n" required>
  <label>${I.nq}</label><input id="q" type="number" min="0" value="0">
  <label>${I.nd}</label><input id="d" type="number" min="0" value="30">
  <label>${I.nsp}</label><input id="s" type="number" min="0" value="0">
  <label>${I.nip}</label><input id="i" type="number" min="0" value="0">
  <button class="btn" style="width:100%;margin-top:14px">${I.mk}</button></form>`);
...
async function showCfg(c){
  ...
  const m=modal(`<h2>${esc(d.name)}</h2>
  ...
  <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap">
    <a class="btn ghost sm" style="padding:8px 14px;border-radius:10px" href="${d.sub_url}" target="_blank">👁 ${I.sub}</a>
...
async function pCfgs(){
  ...
  const rows=d.items.map(c=>{
    ...
    <td data-l="${I.quota}"><div class="bar"><i style="width:${pct}%"></i></div><small style="color:var(--txt-1)">${gb(c.used_bytes)} / ${c.quota_bytes?gb(c.quota_bytes):I.noexp}</small></td>
  ...
  document.querySelectorAll('[data-v]').forEach(b=>b.onclick=async()=>showCfg({id:+b.dataset.v}));
...
async function pGroups(){
  ...
  <button class="btn" id="ng">＋ گروه جدید</button>`);
  ...
  $('#ng').onclick=()=>{
    ...
    <label>${I.gname}</label><input id="n" required>
    <label>${I.gpass}</label><input id="pw" type="password">
    <label style="margin-top:10px">اعضا:</label>${opts||'<p style="color:var(--txt-1)">اول کانفیگ بساز</p>'}
    <button class="btn" style="width:100%;margin-top:14px" ${opts?'':'disabled'}>${I.gmk}</button></form>`);
...
async function pSet(){
  ...
  <div class="card"><h2>${I.chp}</h2><form id="pf">
    ...
    <button class="btn" style="margin-top:14px">${I.save}</button></form></div>
  <div class="card"><h2>${I.vers}</h2><p style="color:var(--neon-1);font-weight:700">3.0.0 — VLESS relay</p></div>`);
...
async function render(){
  ...
  }catch(e){if(e.code==='NO_AUTH')location.hash='#/login';else if(e.message!=='Failed to fetch')toast(e.message,'bad')}
}
